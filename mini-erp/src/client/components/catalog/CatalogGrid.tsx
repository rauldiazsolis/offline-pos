  ...
  if (products.length === 0) {
    return (
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center shadow-sm">
        ...
        <h4 class="text-sm font-bold text-white">No se encontraron productos</h4>
        <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
          Prueba cambiando el término de búsqueda o selecciona otra categoría en la barra de herramientas.
        </p>
      </div>
    );
  }
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm flex flex-col">
      ...
      <div class="px-4 py-2 bg-slate-950/60 border-b border-slate-800 flex items-center justify-between text-[11px] text-slate-400">
        <div class="flex items-center gap-1.5">
          <span class="text-indigo-400 font-bold">💡 Tip Hoja de Cálculo:</span>
        ...
        <table class="w-full text-left border-collapse text-xs">
          <thead>
            <tr class="bg-slate-950/80 border-b border-slate-800 text-[10px] uppercase font-bold tracking-wider text-slate-400 select-none">
              ...
              <th class="py-3 px-4 min-w-[200px]">Nombre / Artículo</th>
              <th class="py-3 px-4 w-36">Categoría</th>
              <th class="py-3 px-4 w-32 text-right">Precio Venta</th>
              <th class="py-3 px-4 w-20 text-center">IVA</th>
              <th class="py-3 px-4 w-28 text-center">Stock</th>
            ...
            {products.map((p) => {
              ...
              return (
                ...
                >
                  {/* SKU */}
                  <td
                    class="py-3 px-4 font-mono text-[11px] text-slate-300 cursor-pointer"
                  ...
                  >
                    {isEditingSku ? (
                      <input
                        ...
                        class="w-full px-2 py-0.5 bg-slate-950 border border-indigo-500 rounded font-mono text-xs text-white focus:outline-none"
                      />
                    ) : (
                      <span class="group-hover:text-indigo-300 transition-colors">{p.sku}</span>
                    )}
                  </td>

                  {/* Nombre */}
                  <td
                    class="py-3 px-4 font-semibold text-white cursor-pointer"
                  ...
                  >
                    {isEditingName ? (
                      <input
                        ...
                        class="w-full px-2 py-0.5 bg-slate-950 border border-indigo-500 rounded text-xs text-white focus:outline-none"
                      />
                    ) : (
                      <div class="flex items-center gap-2">
                        <span class="group-hover:text-indigo-200 transition-colors">{p.name}</span>
                        {p.barcodes.length > 0 && (
                          <span class="text-[10px] text-slate-500 font-mono hidden sm:inline" title="Códigos de barra">
                            🏷️ {p.barcodes[0]}
                          </span>
                        )}
                      </div>
                  ...
                  {/* Categoría */}
                  <td
                    class="py-3 px-4 text-slate-300 cursor-pointer"
                  ...
                  >
                    {isEditingCategory ? (
                      ...
                      <span class="px-2 py-0.5 rounded-md bg-slate-800/80 text-[11px] text-slate-300 border border-slate-700/50 group-hover:border-slate-600 transition-colors">
                        {p.category}
                      </span>
                    )}
                  </td>
                  ...
                  {/* Precio Venta */}
                  <td
                    class="py-3 px-4 text-right font-mono font-bold text-white cursor-pointer"
                  ...
                  >
                    {isEditingPrice ? (
                      <input
                        ...
                        class="w-24 px-2 py-0.5 bg-slate-950 border border-indigo-500 rounded font-mono text-xs text-right text-emerald-400 focus:outline-none"
                      />
                    ) : (
                      <span class="text-emerald-400 group-hover:underline decoration-emerald-500/50 underline-offset-2">
                  ...
                  <td class="py-3 px-4 text-center">
                    {!p.tracksStock ? (
                      <span class="text-[10px] text-slate-500 font-mono">Infinito</span>
                    ) : stock === undefined ? (
