...
export type InputProps = Omit<JSX.IntrinsicElements['input'], 'class' | 'className'> & {
  class?: string;
  className?: string;
  label?: string;
  error?: string | null;
  ...
  return (
    <div class="w-full space-y-1.5 text-left">
      ...
      <input
        ...
        } rounded-xl text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-offset-white dark:focus:ring-offset-slate-950 transition-all${extraClass}`}
        {...rest}
      />
