    ...
    <div class="space-y-6">
      ...
      <Card class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          {/* Entidad */}
          <div>
            <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Entidad a Importar *</label>
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <label class="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 underline cursor-pointer font-medium">
              Cargar archivo .csv
          ...
          <textarea
            ...
            class="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-mono text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          ></textarea>
        </div>
        ...
        {preview && (
          <div class="p-4 rounded-xl border bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 space-y-3 animate-in fade-in duration-150">
            <div class="flex items-center gap-4 text-xs">
              <span class="font-bold text-slate-900 dark:text-white">
                {preview.dryRun ? 'Resultado de la Simulación:' : 'Importación Finalizada:'}
              </span>
              <span class="text-emerald-600 dark:text-emerald-400 font-mono font-bold">Nuevos: {preview.importedCount}</span>
              <span class="text-indigo-600 dark:text-indigo-400 font-mono font-bold">Actualizados: {preview.updatedCount}</span>
              <span class="text-slate-600 dark:text-slate-400 font-mono">Omitidos: {preview.skippedCount}</span>
              {preview.errors.length > 0 && (
                <span class="text-rose-600 dark:text-rose-400 font-bold font-mono">Errores: {preview.errors.length}</span>
              )}
            </div>

            {preview.errors.length > 0 && (
              <div class="space-y-1.5 max-h-40 overflow-y-auto pt-2 border-t border-slate-200 dark:border-slate-800/80">
                <div class="text-[11px] font-bold text-rose-600 dark:text-rose-400">Filas con errores detectados:</div>
                {preview.errors.map((err, idx) => (
                  <div key={idx} class="text-xs text-rose-700 dark:text-rose-300 font-mono bg-rose-500/10 px-2.5 py-1 rounded-md">
