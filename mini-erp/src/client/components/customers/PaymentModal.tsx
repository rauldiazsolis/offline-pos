...
export function PaymentModal() {
  if (!paymentModalOpenSignal.value) return null;
      ...
      <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div class="p-6 border-b border-slate-800 bg-slate-950/40 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-2xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center text-lg font-bold">
              💵
            </div>
            <div>
              <h3 class="text-base font-bold text-white">Registrar Cobranza</h3>
              <p class="text-xs text-slate-400">Ingreso de pago en cuenta corriente</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="p-3.5 bg-slate-950/60 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
            ...
            <div class="text-right">
              <div class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Saldo Adeudado</div>
              <div class="text-base font-black font-mono text-rose-400 mt-0.5">
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              <label class="block text-xs font-medium text-slate-300">Importe a Cobrar ($ ARS) *</label>
              {target.balance > 0 && (
                <button
                  type="button"
                  onClick={() => updateField('amount', target.balance)}
                  class="text-[11px] text-emerald-400 hover:text-emerald-300 underline cursor-pointer"
                >
                  Cobrar saldo total
                </button>
              )}
            ...
            <input
              ...
              class="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs font-mono font-bold text-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              autoFocus
            />
          </div>
          ...
          {/* Medio de Pago */}
          <div>
            <label class="block text-xs font-medium text-slate-300 mb-1.5">Medio de Pago</label>
            <select
              value={form.method}
              onChange={(e) => updateField('method', (e.target as HTMLSelectElement).value)}
              class="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer"
            >
